...
//
int*
my_realloc(int *arr, const size_t size_arr, const size_t new_size) {
...
//
bool
is_anagram(string str1, string str2) {
  remove_char(str1, ' ');
  remove_char(str2, ' ');
...
//
int
is_sorted_type(int *array, const int size) {
  bool crescent = true;
...
//
double
harmonics(const unsigned int n) {
  if (n == 0) return 0.0;
...
//
unsigned long
sum_factorial(const unsigned int n) {
...
//
unsigned int
freq_vector(const int *vec, const size_t vec_size, const int x) {
  unsigned int freq = 0;
...
main(int argc, char const *argv[]) {
  ...
  puts("\nEXERCICIO ..");
  n = 5;
  printf("Soma dos fatoriais de %2d! = %d\n", n, sum_factorial(n));
  n = 10;
  printf("Soma dos fatoriais de %2d! = %d\n", n, sum_factorial(n));
